  ...
  useEffect(() => {
    try {
      ...
      if (data) {
        ...
        // Preload default demo account matching user's image!
        const initialAccounts: SavedAccount[] = [
          {
            id: 'default-vcb',
            bankBin: '970436',
            bankCode: 'VCB',
            accountNumber: '0611001946522',
            accountName: 'Lam Thanh Son',
            label: 'Tài khoản chính (VCB)',
            createdAt: Date.now()
          }
        ];
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none mb-3">
        {savedAccounts.map((acc) => {
          ...
          return (
            ...
            >
              <span className="font-mono text-[11px] font-bold text-emerald-700">{acc.bankCode}</span>
        ...
        {currentAccountNumber && !isCurrentSaved && (
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold bg-emerald-600 text-white hover:bg-emerald-700 transition-all shrink-0 shadow-2xs"
          >
        ...
        {justSaved && (
          <span className="text-xs text-emerald-600 flex items-center gap-1 shrink-0 font-medium">
          ...
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-slate-200 overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/80">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-slate-900 text-sm">Danh sách tài khoản ngân hàng của bạn</h3>
              </div>
              <button
                type="button"
                ...
                savedAccounts.map((acc) => {
                  ...
                  return (
                    <div
                      ...
                      className="flex items-center justify-between p-3 rounded-xl border border-slate-200 hover:border-emerald-400 hover:bg-emerald-50/40 cursor-pointer transition-all group"
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          {acc.accountName && (
                            <div className="text-[11px] text-slate-600 uppercase">
                              {acc.accountName}
                            </div>
                          )}
                        </div>
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-1.5 text-xs bg-white border border-slate-200 rounded-lg focus:outline-hidden focus:border-emerald-500"
                />
                <button
                  type="button"
                  onClick={handleSaveCurrent}
