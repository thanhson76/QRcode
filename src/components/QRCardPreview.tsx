...
export const QRCardPreview: React.FC<QRCardPreviewProps> = ({
  ...
  return (
    <div className="flex flex-col items-center w-full">
      {/* View style tabs */}
      <div className="flex items-center gap-1 p-1 bg-slate-200/80 rounded-xl mb-4 w-full max-w-md text-xs font-medium">
        <button
          type="button"
      ...
      <div
        ref={cardRef}
        id="qr-preview-card"
        className="w-full max-w-md bg-white rounded-2xl border border-slate-200 shadow-lg overflow-hidden transition-all print:border-none print:shadow-none print:m-0"
      >
          ...
          <div className="p-4 sm:p-5">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] text-slate-600 font-medium border-l border-slate-200 pl-2">
                  Chuẩn Napas 247
                </span>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-center">
              ...
              <div className="sm:col-span-7 space-y-2.5 text-xs">
                <div className="bg-slate-50/80 rounded-xl p-2.5 border border-slate-100">
                  <div className="text-[11px] text-slate-600 mb-0.5">Tên ngân hàng</div>
                ...
                <div className="bg-slate-50/80 rounded-xl p-2.5 border border-slate-100">
                  <div className="text-[11px] text-slate-600 mb-0.5">Tên người nhận</div>
                  <div className="font-bold text-slate-900 uppercase text-sm">
                    {cleanAccountName || <span className="text-slate-400 font-normal">Chưa nhập tên</span>}
                  </div>
                </div>

                <div className="bg-slate-50/80 rounded-xl p-2.5 border border-slate-100">
                  <div className="text-[11px] text-slate-600 mb-0.5">Số tài khoản</div>
                  <div className="font-mono font-bold text-emerald-700 text-base tracking-wide">
                    {accountNumber || <span className="text-slate-400 font-normal font-sans text-xs">Chưa nhập STK</span>}
                  </div>
                </div>

                {rawAmount > 0 && (
                  <div className="bg-emerald-50/70 rounded-xl p-2.5 border border-emerald-100">
                    <div className="text-[11px] text-emerald-800 font-medium mb-0.5">Số tiền</div>
                    <div className="font-bold text-emerald-900 text-base">
                      {formattedAmount} <span className="text-xs font-normal">VND</span>
                    </div>
                    {amountWords && (
                      <div className="text-[10px] text-emerald-800 italic mt-0.5 leading-tight">
                        ({amountWords})
                      </div>
                    )}
                  </div>
                ...
                {description && (
                  <div className="bg-slate-50/80 rounded-xl p-2.5 border border-slate-100">
                    <div className="text-[11px] text-slate-600 mb-0.5">Nội dung chuyển khoản</div>
              ...
              {/* Right: QR Code */}
              <div className="sm:col-span-5 flex flex-col items-center justify-center p-2 bg-slate-50 rounded-xl border border-slate-100">
                ...
                <div className="flex items-center justify-between w-full px-2 mt-2 text-[10px] text-slate-600">
                  <span className="font-bold text-red-600">VIET<span className="text-blue-600">QR</span></span>
                  <span className="font-bold text-blue-700">napas<span className="text-emerald-600">247</span></span>
                </div>
              </div>
            </div>
          </div>
        ...
        {displayStyle === 'standee' && (
          <div className="p-6 text-center bg-radial from-white via-slate-50 to-slate-100/50">
            {/* Store header badge */}
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-100 text-emerald-800 text-xs font-bold mb-2 uppercase tracking-wide">
              <Store className="w-3.5 h-3.5" />
            ...
            {/* Bank details pill */}
            <div className="bg-white rounded-xl p-3 border border-slate-200/90 shadow-xs max-w-xs mx-auto text-left space-y-1">
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-600">Ngân hàng:</span>
                <span className="font-bold text-slate-900">{bank.shortName}</span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-600">Số tài khoản:</span>
                <span className="font-mono font-bold text-emerald-700 text-sm">{accountNumber || '---'}</span>
              </div>
              {rawAmount > 0 && (
                <div className="flex justify-between items-center text-xs pt-1 border-t border-slate-100">
                  <span className="text-slate-600">Số tiền:</span>
                  <span className="font-bold text-emerald-800 text-sm">{formattedAmount} đ</span>
                </div>
              )}
              {description && (
                <div className="flex justify-between items-center text-xs pt-1 border-t border-slate-100">
                  <span className="text-slate-600">Nội dung:</span>
                  <span className="font-medium text-slate-800 truncate max-w-[150px]">{description}</span>
                </div>
              )}
            </div>

            <div className="mt-4 flex items-center justify-center gap-4 text-[11px] font-bold text-slate-600">
              <span className="text-red-600">VIETQR</span>
              <span>•</span>
              <span className="text-blue-700">NAPAS 247</span>
              <span>•</span>
              <span className="text-emerald-700">CHUYỂN TIỀN NHANH</span>
            </div>
          </div>
        )}
        ...
        {displayStyle === 'compact' && (
          <div className="p-6 flex flex-col items-center justify-center bg-white text-center">
            <div className="w-64 h-64 bg-white p-3 rounded-2xl border border-slate-200 shadow-md flex items-center justify-center mb-3">
              {!imgError && hasData ? (
                <img
                  src={displayQrUrl}
                  alt="VietQR"
            ...
            <div className="text-xs text-slate-600">
              ...
              <span className="font-mono text-emerald-700 font-bold">{accountNumber || 'Chưa nhập'}</span>
            </div>
            {rawAmount > 0 && (
      ...
      <div className="w-full max-w-md mt-4 space-y-2 print:hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          ...
          <button
            ...
            disabled={!hasData || isDownloading}
            className="flex items-center justify-center gap-2 py-3 px-3 bg-white border border-slate-300 hover:bg-slate-50 active:bg-slate-100 text-slate-800 font-bold text-xs sm:text-sm rounded-xl shadow-2xs transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
        ...
        <div className="grid grid-cols-3 gap-2">
          <button
            ...
            disabled={!hasData}
            className="flex items-center justify-center gap-1.5 py-2.5 px-3 bg-white border border-slate-200 hover:bg-slate-50 rounded-xl text-xs font-semibold text-slate-700 shadow-2xs transition-all disabled:opacity-50"
      ...
      {/* Guide notice */}
      <div className="w-full max-w-md mt-4 p-3 bg-slate-100/70 border border-slate-200/80 rounded-xl text-[11px] text-slate-600 flex items-start gap-2.5 print:hidden">
        <ShieldCheck className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
        <div>
          <span className="font-semibold text-slate-700">Tiêu chuẩn VietQR Quốc gia:</span> Mã QR tạo ra tương thích 100% với tất cả ứng dụng Mobile Banking của 50+ ngân hàng tại Việt Nam. Người thanh toán chỉ cần mở app ngân hàng quét mã là xong.
        </div>
      </div>
    </div>
  );
